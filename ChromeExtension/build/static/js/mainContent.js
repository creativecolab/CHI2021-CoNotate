...
if(this.suggestionsWidth-containerShiftLeft>=this.clientWidth)this.nextBtn.hidden=false;else this.nextBtn.hidden=true;}},{key:"updateOnSuggestionDelete",value:function updateOnSuggestionDelete(){var _this2=this;var queryStart=this.autoComplete.join(' ');var firstPrevious=false;var _loop=function _loop(i){var element=_this2.container.children[i];if(!firstPrevious&&element.classList.contains("previous")){element.classList.add("breaker");firstPrevious=true;}if(!firstPrevious){element.onclick=function(){window.location.href="https://www.google.com/search?q=".concat(queryStart," ").concat(_this2.container.children[i].innerHTML);};}else{element.childNodes[1].onclick=function(e){e.cancelBubble=true;if(e.stopPropagation)e.stopPropagation();_this2.autoComplete.splice(i,1);chromePort["a" /* default */].postMessage({type:"deleteSuggestion",data:i});_this2.suggestionsWidth-=element.offsetWidth;_this2.container.removeChild(element);_this2.updateOnSuggestionDelete();};}};for(var i=this.container.children.length-1;i>=0;i--){_loop(i);}}},{key:"createPreviousSuggestionNode",value:function createPreviousSuggestionNode(){var _this3=this;var text=arguments.length>0&&arguments[0]!==undefined?arguments[0]:"";var cssClass=arguments.length>1&&arguments[1]!==undefined?arguments[1]:[];var onClick=arguments.length>2&&arguments[2]!==undefined?arguments[2]:null;var i=arguments.length>3&&arguments[3]!==undefined?arguments[3]:0;var element=document.createElement("div");for(var _i=0;_i<cssClass.length;_i++){element.classList.add(cssClass[_i]);}if(onClick)element.onclick=onClick;var elementContent=document.createElement("span");var elementContentText=document.createTextNode(text);elementContent.append(elementContentText);element.append(elementContent);var elementDelete=document.createElement("span");var elementDeleteText=document.createTextNode("x");elementDelete.append(elementDeleteText);elementDelete.classList.add("delete");elementDelete.onclick=function(e){e.cancelBubble=true;if(e.stopPropagation)e.stopPropagation();_this3.autoComplete.splice(i,1);chromePort["a" /* default */].postMessage({type:"deleteSuggestion",data:i});_this3.suggestionsWidth-=element.offsetWidth;_this3.container.removeChild(element);_this3.updateOnSuggestionDelete();};element.append(elementDelete);this.container.append(element);return element;}},{key:"createDOMNode",value:function createDOMNode(){var text=arguments.length>0&&arguments[0]!==undefined?arguments[0]:"";var cssClass=arguments.length>1&&arguments[1]!==undefined?arguments[1]:[];var onClick=arguments.length>2&&arguments[2]!==undefined?arguments[2]:null;var element=document.createElement("div");for(var i=0;i<cssClass.length;i++){element.classList.add(cssClass[i]);}if(onClick)element.onclick=onClick;var elementText=document.createTextNode(text);element.append(elementText);this.container.append(element);return element;}},{key:"connectedCallback",value:function connectedCallback(){var _this4=this;this.nextBtn.addEventListener('click',this.next);this.previousBtn.addEventListener('click',this.previous);this.clientWidth=document.documentElement.clientWidth;chrome.runtime.onMessage.addListener(function(msg){if(msg.type==="processDoc"){// Reset the width of the suggestions
var totalSuggestionWidth=0;// Clear the previous suggestions if they are there
while(_this4.container.firstChild){_this4.container.removeChild(_this4.container.lastChild);}if(msg.data.suggestionsCache.length===0||msg.data.suggestionsCache.length<_this4.length&&_this4.emptyMessage!=="There are no valid suggestions"){_this4.createDOMNode(_this4.emptyMessage,["extension-text"]);}else{(function(){var queryStart=msg.data.autoComplete.join(' ').trim();_this4.autoComplete=msg.data.autoComplete;var _loop2=function _loop2(i){var filteredData=msg.data.autoComplete[i];totalSuggestionWidth+=_this4.createPreviousSuggestionNode(filteredData,["extension-suggestion","previous"],function(){window.location.href="https://www.google.com/search?q=".concat(filteredData);},i// The 8 is the margin width
).offsetWidth+8;};for(var i=0;i<msg.data.autoComplete.length-1;i++){_loop2(i);}var breaker=msg.data.autoComplete[msg.data.autoComplete.length-1];totalSuggestionWidth+=_this4.createPreviousSuggestionNode(breaker,["extension-suggestion","previous","breaker"],function(){window.location.href="https://www.google.com/search?q=".concat(breaker);},msg.data.autoComplete.length-1// The 24 is the margin width for the breaker
).offsetWidth+24;var _loop3=function _loop3(j){var filteredData=msg.data.suggestionsCache[j].replaceAll('%20',' ');totalSuggestionWidth+=_this4.createDOMNode(filteredData,["extension-suggestion"],function(){window.location.href="https://www.google.com/search?q=".concat(queryStart," ").concat(filteredData);}// The 8 is the margin width
).offsetWidth+8;};for(var j=msg.data.suggestionsCache.length-1;j>=0;--j){_loop3(j);}_this4.suggestionsWidth=totalSuggestionWidth;})();}// Prep the suggestion container
_this4.checkBtn();}});// Prevent default scrolling behavior when hovered over the top menu
this.scrollEvent=function(e){if(stopScroll){e.preventDefault();e.stopPropagation();if(e.wheelDeltaY<0)_this4.next(100);else _this4.previous(100);}};this.resizeEvent=function(){var previousClientWidth=_this4.clientWidth;_this4.clientWidth=document.documentElement.clientWidth;// While this may seem weird, we call the call the previous function to make sure we adapt to the range
if(parseFloat(_this4.container.style.left)!==0)_this4.previous(_this4.clientWidth-previousClientWidth);_this4.checkBtn();};window.addEventListener("resize",this.resizeEvent);window.addEventListener('wheel',this.scrollEvent,{passive:false});}},{key:"disconnectedCallback",value:function disconnectedCallback(){this.nextBtn.removeEventListener('click',this.next);this.previousBtn.removeEventListener('click',this.previous);window.removeEventListener("resize",this.resizeEvent);window.removeEventListener('wheel',this.scrollEvent,{passive:false});}},{key:"emptyMessage",get:function get(){return this.getAttribute('emptyMessage');},set:function set(value){this.setAttribute('emptyMessage',value);}},{key:"clientWidth",get:function get(){return this.getAttribute('clientWidth');},set:function set(value){this.setAttribute('clientWidth',value);}},{key:"length",get:function get(){return this.getAttribute('length');},set:function set(value){this.setAttribute('length',value);}}],[{key:"observedAttributes",get:function get(){return['clientWidth','emptyMessage','length'];}}]);return SuggestionContainer;}(/*#__PURE__*/wrapNativeSuper_wrapNativeSuper(HTMLElement)));// Immediate invokation since we want top-level async
...
var checkContainer=function checkContainer(container,children,callback){container.forEach(function(container){return children.forEach(function(child){return[container.getElementsByClassName(child)].forEach(function(child){return Object(toConsumableArray["a" /* default */])(child).forEach(function(child){return callback(child);});});});});};var timeStamp=Date.now();var scrapeDivs=function scrapeDivs(divs){var scrapeObj={};var keys=Object.keys(divs);var _loop4=function _loop4(j){var _divs$keys$j=Object(slicedToArray["a" /* default */])(divs[keys[j]],3),container=_divs$keys$j[0],children=_divs$keys$j[1],watch=_divs$keys$j[2];if(container){if(container.length)container=Object(toConsumableArray["a" /* default */])(container);else container=[container];if(watch){var textArr=[];checkContainer(container,children,function(ele){return textArr.push({text:ele.innerText,open:false});});scrapeObj[keys[j]]=textArr;// Listener for when the div changes
// We can prevent sending a bunch of data with a debouncer and this data is not urgent
var timer=null,cancelTimer=null;container.forEach(function(subContainer){return Object(utility["a" /* observeDOM */])(subContainer,function(){if(cancelTimer)cancelTimer();timer=setTimeout(function(){var textArr=[];checkContainer(container,children,function(ele){var text=ele.innerText;textArr.push({text:text,open:text.indexOf("\n")!==-1?true:false});});chromePort["a" /* default */].postMessage({type:"updatePeopleAlsoAsk",data:{timeStamp:timeStamp,peopleAlsoAsk:textArr}});},500);cancelTimer=function cancelTimer(){return clearTimeout(timer);};});});}else{var _textArr=[];checkContainer(container,children,function(ele){return _textArr.push(ele.innerText);});scrapeObj[keys[j]]=_textArr;}}else scrapeObj[keys[j]]=[];};for(var j=keys.length-1;j>=0;--j){_loop4(j);}return scrapeObj;};// Scrape the divs to send to the background script
