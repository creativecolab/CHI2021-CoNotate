...
// Module
var code = "<style>button{position:fixed;background:#2d2d2d;color:#fff;border:none;padding:0;height:36px;width:60px;font:inherit;cursor:pointer;outline:inherit;z-index:2147483647;display:flex;font-size:20px;align-items:center;justify-content:center}[previous]{top:0;left:0;transform:rotate(180deg)}[next]{top:0;right:0}[hidden]{display:none!important}#extension-suggestions-container{position:relative;font-family:'Public Sans',sans-serif;line-height:16px;font-size:20px;height:36px;left:0;white-space:nowrap;transition:left 1s;color:#fff}.extension-suggestion:hover{background:#d8d8d8;border:#d8d8d8 2px solid}.extension-suggestion{font-size:14px;display:inline-block;padding:3px 8px;margin-left:8px;margin-top:5px;color:#606060;background:#f2f2f2;border:#e7e7e7 2px solid;border-radius:15px;text-align:center;align-items:center;cursor:pointer}.breaker{margin-right:16px;position:relative}.breaker::after{content:'';position:absolute;top:-10px;right:-16px;width:4px;height:50px;background:#fff;pointer-events:none}.extension-text{display:flex;align-items:center;justify-content:center;height:100%}</style> <button hidden previous>></button> <div id=\"extension-suggestions-container\" style=\"left:0\"> <div class=\"extension-text\">Loading...</div> </div> <button hidden next>></button>";
// Exports
module.exports = code;
...
// EXTERNAL MODULE: ./src/helpers/utility.js
var utility = __webpack_require__(23);

// EXTERNAL MODULE: ./node_modules/@webcomponents/custom-elements/custom-elements.min.js
var custom_elements_min = __webpack_require__(44);

// EXTERNAL MODULE: ./src/mainContent.html
var mainContent = __webpack_require__(32);
var mainContent_default = /*#__PURE__*/__webpack_require__.n(mainContent);

// CONCATENATED MODULE: ./src/mainContent.js
/* global chrome */ /* eslint-disable no-undef */// Preperation for our scroll events
var stopScroll=false;// Create our plugin template from an import
var suggestionContainerTemplate=document.createElement('template');suggestionContainerTemplate.innerHTML=mainContent_default.a;// Define the script for our template
window.customElements.define('suggestion-container',/*#__PURE__*/function(_HTMLElement){_inherits(SuggestionContainer,_HTMLElement);var _super=_createSuper(SuggestionContainer);function SuggestionContainer(){var _this;Object(classCallCheck["a" /* default */])(this,SuggestionContainer);_this=_super.call(this);_this.next=_this.next.bind(_assertThisInitialized(_this));_this.previous=_this.previous.bind(_assertThisInitialized(_this));_this.suggestionsWidth=0;// Use our suggestion template
_this.attachShadow({mode:'open'});_this.shadowRoot.appendChild(suggestionContainerTemplate.content.cloneNode(true));_this.nextBtn=_this.shadowRoot.querySelector('[next]');_this.previousBtn=_this.shadowRoot.querySelector('[previous]');_this.container=_this.shadowRoot.querySelector('div');return _this;}Object(createClass["a" /* default */])(SuggestionContainer,[{key:"next",value:function next(value){if(value<0)return;// The 120 is to account for the two arrow signs
if(_typeof(value)==='object')value=this.clientWidth-120;var widthDifference=-1*(this.suggestionsWidth-this.clientWidth+8);if(widthDifference>0)return;var newLeft;this.container.style.left="\n                ".concat(widthDifference>(newLeft=parseFloat(this.container.style.left)-parseFloat(value))?widthDifference:newLeft,"px\n            ");this.checkBtn();}},{key:"previous",value:function previous(value){if(value<0)return;// The 120 is to account for the two arrow signs
if(_typeof(value)==='object')value=this.clientWidth-120;var newLeft;this.container.style.left="\n                ".concat((newLeft=parseFloat(this.container.style.left)+parseFloat(value))>0?0:newLeft,"px\n            ");this.checkBtn();}},{key:"checkBtn",value:function checkBtn(){var containerShiftLeft=-1*parseFloat(this.container.style.left);// We don't need to check for positive or negative as our previous and next functions already handle that
if(containerShiftLeft===0)this.previousBtn.hidden=true;else this.previousBtn.hidden=false;// The +8 is to account for the margin on the last one we added (kinda weird but more effective)
if(this.suggestionsWidth-containerShiftLeft>=this.clientWidth)this.nextBtn.hidden=false;else this.nextBtn.hidden=true;}},{key:"createDOMNode",value:function createDOMNode(text,cssClass){var onClick=arguments.length>2&&arguments[2]!==undefined?arguments[2]:null;var element=document.createElement("div");for(var i=0;i<cssClass.length;i++){element.classList.add(cssClass[i]);}if(onClick)element.onclick=onClick;var elementText=document.createTextNode(text);element.append(elementText);this.container.append(element);return element;}},{key:"connectedCallback",value:function connectedCallback(){var _this2=this;this.nextBtn.addEventListener('click',this.next);this.previousBtn.addEventListener('click',this.previous);this.clientWidth=document.documentElement.clientWidth;chrome.runtime.onMessage.addListener(function(msg){if(msg.type==="processDoc"){// Reset the width of the suggestions
var totalSuggestionWidth=0;console.log(msg);// Clear the previous suggestions if they are there
while(_this2.container.firstChild){_this2.container.removeChild(_this2.container.lastChild);}if(msg.data.suggestionsCache.length===0||msg.data.suggestionsCache.length<_this2.length&&_this2.emptyMessage!=="There are no valid suggestions"){_this2.createDOMNode(_this2.emptyMessage,["extension-text"]);}else{(function(){var queryStart=msg.data.autoComplete.join('');var _loop=function _loop(i){var filteredData=msg.data.autoComplete[i];totalSuggestionWidth+=_this2.createDOMNode(filteredData,["extension-suggestion"],function(){window.location.href="https://www.google.com/search?q=".concat(filteredData);}// The 8 is the margin width
).offsetWidth+8;};for(var i=0;i<msg.data.autoComplete.length-1;i++){_loop(i);}var breaker=msg.data.autoComplete[msg.data.autoComplete.length-1];totalSuggestionWidth+=_this2.createDOMNode(breaker,["extension-suggestion","breaker"],function(){window.location.href="https://www.google.com/search?q=".concat(breaker);}// The 24 is the margin width for the breaker
).offsetWidth+24;var _loop2=function _loop2(j){var filteredData=msg.data.suggestionsCache[j].replaceAll('%20',' ');totalSuggestionWidth+=_this2.createDOMNode(filteredData,["extension-suggestion"],function(){window.location.href="https://www.google.com/search?q=".concat(queryStart," ").concat(filteredData);}// The 8 is the margin width
).offsetWidth+8;};for(var j=msg.data.suggestionsCache.length-1;j>=0;--j){_loop2(j);}_this2.suggestionsWidth=totalSuggestionWidth;})();}// Prep the suggestion container
_this2.checkBtn();}});// Prevent default scrolling behavior when hovered over the top menu
this.scrollEvent=function(e){if(stopScroll){e.preventDefault();e.stopPropagation();if(e.wheelDeltaY<0)_this2.next(100);else _this2.previous(100);}};this.resizeEvent=function(){var previousClientWidth=_this2.clientWidth;_this2.clientWidth=document.documentElement.clientWidth;// While this may seem weird, we call the call the previous function to make sure we adapt to the range
if(parseFloat(_this2.container.style.left)!==0)_this2.previous(_this2.clientWidth-previousClientWidth);_this2.checkBtn();};window.addEventListener("resize",this.resizeEvent);window.addEventListener('wheel',this.scrollEvent,{passive:false});}},{key:"disconnectedCallback",value:function disconnectedCallback(){this.nextBtn.removeEventListener('click',this.next);this.previousBtn.removeEventListener('click',this.previous);window.removeEventListener("resize",this.resizeEvent);window.removeEventListener('wheel',this.scrollEvent,{passive:false});}},{key:"emptyMessage",get:function get(){return this.getAttribute('emptyMessage');},set:function set(value){this.setAttribute('emptyMessage',value);}},{key:"clientWidth",get:function get(){return this.getAttribute('clientWidth');},set:function set(value){this.setAttribute('clientWidth',value);}},{key:"length",get:function get(){return this.getAttribute('length');},set:function set(value){this.setAttribute('length',value);}}],[{key:"observedAttributes",get:function get(){return['clientWidth','emptyMessage','length'];}}]);return SuggestionContainer;}(/*#__PURE__*/wrapNativeSuper_wrapNativeSuper(HTMLElement)));// Immediate invokation since we want top-level async
Object(asyncToGenerator["a" /* default */])(/*#__PURE__*/regenerator_default.a.mark(function _callee(){var peopleAskContainer,peopleSearchContainer,searchRelatedContainer,queries,_yield$Promise,_yield$Promise2,pluginMode,pluginLength,pluginToggleState,pluginHeader,pluginToggle;return regenerator_default.a.wrap(function _callee$(_context){while(1){switch(_context.prev=_context.next){case 0:// Containers we want to scrape (and maybe delete)
peopleAskContainer=document.getElementsByClassName("JolIg mfMhoc")[0]?document.getElementsByClassName("JolIg mfMhoc")[0].parentNode:null;peopleSearchContainer=document.getElementsByClassName("lgJJud")[0];searchRelatedContainer=document.getElementsByClassName("A07Bwc")[0];queries=document.getElementsByClassName("g").length>0?document.getElementsByClassName("g"):null;// Connect to the background script
_context.next=6;return new Promise(function(success){// Start the listener for the background script before we scrape the divs and connect
// After getting the data from the connection close the listener
chromePort["a" /* default */].onMessage.addListener(function handleConnection(msg){if(msg.type==="connection"&&msg.data.frame==="parent"&&msg.data.state.isMain){// Remove google additional information if experimental
if(msg.data.state.PLUGIN_MODE==="experimental_notes"){var removeDivs=function removeDivs(divs){for(var j=divs.length-1;j>=0;--j){if(divs[j])divs[j].parentNode.removeChild(divs[j]);}};removeDivs([peopleAskContainer,peopleSearchContainer,searchRelatedContainer]);}// Make sure we stop listening once we recieve the connection
chromePort["a" /* default */].onMessage.removeListener(handleConnection);// Return our data
success([msg.data.state.PLUGIN_MODE,msg.data.state.PLUGIN_TYPE.length*3,msg.data.state.toggleOpen]);return;}});// Functions to help us scrape the divs
var checkContainer=function checkContainer(container,children,callback){container.forEach(function(container){return children.forEach(function(child){return[container.getElementsByClassName(child)].forEach(function(child){return Object(toConsumableArray["a" /* default */])(child).forEach(function(child){return callback(child);});});});});};var timeStamp=Date.now();var scrapeDivs=function scrapeDivs(divs){var scrapeObj={};var keys=Object.keys(divs);var _loop3=function _loop3(j){var _divs$keys$j=Object(slicedToArray["a" /* default */])(divs[keys[j]],3),container=_divs$keys$j[0],children=_divs$keys$j[1],watch=_divs$keys$j[2];if(container){if(container.length)container=Object(toConsumableArray["a" /* default */])(container);else container=[container];if(watch){var textArr=[];checkContainer(container,children,function(ele){return textArr.push({text:ele.innerText,open:false});});scrapeObj[keys[j]]=textArr;// Listener for when the div changes
// We can prevent sending a bunch of data with a debouncer and this data is not urgent
var timer=null,cancelTimer=null;container.forEach(function(subContainer){return Object(utility["a" /* observeDOM */])(subContainer,function(){if(cancelTimer)cancelTimer();timer=setTimeout(function(){var textArr=[];checkContainer(container,children,function(ele){var text=ele.innerText;textArr.push({text:text,open:text.indexOf("\n")!==-1?true:false});});chromePort["a" /* default */].postMessage({type:"updatePeopleAlsoAsk",data:{timeStamp:timeStamp,peopleAlsoAsk:textArr}});},500);cancelTimer=function cancelTimer(){return clearTimeout(timer);};});});}else{var _textArr=[];checkContainer(container,children,function(ele){return _textArr.push(ele.innerText);});scrapeObj[keys[j]]=_textArr;}}else scrapeObj[keys[j]]=[];};for(var j=keys.length-1;j>=0;--j){_loop3(j);}return scrapeObj;};// Scrape the divs to send to the background script
var scrapeObj;if(document.location.href.includes('https://www.google.com/search')){scrapeObj=scrapeDivs({scrapeSerp:[queries,["LC20lb","st"],false],peopleAlsoAsk:[peopleAskContainer,["related-question-pair"],true],peopleAlsoSearch:[peopleSearchContainer,["f3LoEf","iKt1P"],false],relatedSearches:[searchRelatedContainer,["s75CSd"],false]});scrapeObj['scrapeSerp']=scrapeObj['scrapeSerp'].filter(function(x){return x!=='';}).join();}// Connect to the background script
chromePort["a" /* default */].postMessage({type:"connection",data:Object(objectSpread2["a" /* default */])({url:document.location.href,timeStamp:timeStamp},scrapeObj)});});case 6:_yield$Promise=_context.sent;_yield$Promise2=Object(slicedToArray["a" /* default */])(_yield$Promise,3);pluginMode=_yield$Promise2[0];pluginLength=_yield$Promise2[1];pluginToggleState=_yield$Promise2[2];// Post connection code
// Create the plugin topbar
pluginHeader=document.createElement("suggestion-container");pluginHeader.id="extension-header";pluginHeader.emptyMessage=pluginMode==="experimental_notes"?"Please add to your notes to get more suggestions":"There are no valid suggestions";pluginHeader.length=pluginLength;pluginHeader.onmouseover=function(){stopScroll=true;};pluginHeader.onmouseout=function(){stopScroll=false;};document.getElementsByTagName('html')[0].append(pluginHeader);document.getElementsByTagName('body')[0].style.transform="translateY(40px)";// Create the toggle button
pluginToggle=document.createElement("div");pluginToggle.id="extension-toggle";pluginToggle.onclick=function(){pluginToggleState=!pluginToggleState;pluginToggle.innerHTML="".concat(pluginToggleState?'>':'<');chromePort["a" /* default */].postMessage({type:"toggleResize"});};pluginToggle.innerHTML="".concat(pluginToggleState?'>':'<');document.getElementsByTagName('html')[0].append(pluginToggle);case 24:case"end":return _context.stop();}}},_callee);}))();
