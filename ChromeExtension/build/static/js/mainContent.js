...
if(this.suggestionsWidth-containerShiftLeft>=this.clientWidth)this.nextBtn.hidden=false;else this.nextBtn.hidden=true;}},{key:"createDOMNode",value:function createDOMNode(text,cssClass){var onClick=arguments.length>2&&arguments[2]!==undefined?arguments[2]:null;var element=document.createElement("div");element.classList.add(cssClass);if(onClick)element.onclick=onClick;var elementText=document.createTextNode(text);element.append(elementText);this.container.append(element);return element;}},{key:"connectedCallback",value:function connectedCallback(){var _this2=this;this.nextBtn.addEventListener('click',this.next);this.previousBtn.addEventListener('click',this.previous);this.clientWidth=document.documentElement.clientWidth;chrome.runtime.onMessage.addListener(function(msg){if(msg.type==="processDoc"){// Reset the width of the suggestions
var totalSuggestionWidth=0;// Clear the previous suggestions if they are there
while(_this2.container.firstChild){_this2.container.removeChild(_this2.container.lastChild);}if(msg.data.length===0||msg.data.length<_this2.length&&_this2.emptyMessage!=="There are no valid suggestions"){_this2.createDOMNode(_this2.emptyMessage,"extension-text");}else{var _loop=function _loop(j){var filteredData=msg.data[j].replaceAll('%20',' ');totalSuggestionWidth+=_this2.createDOMNode(filteredData,"extension-suggestion",function(){window.location.href="https://www.google.com/search?q=".concat(filteredData);}// The 8 is the margin width
).offsetWidth+8;};for(var j=msg.data.length-1;j>=0;--j){_loop(j);}_this2.suggestionsWidth=totalSuggestionWidth;}// Prep the suggestion container
...
var checkContainer=function checkContainer(container,children,callback){container.forEach(function(container){return children.forEach(function(child){return[container.getElementsByClassName(child)].forEach(function(child){return Object(toConsumableArray["a" /* default */])(child).forEach(function(child){return callback(child);});});});});};var timeStamp=Date.now();var scrapeDivs=function scrapeDivs(divs){var scrapeObj={};var keys=Object.keys(divs);var _loop2=function _loop2(j){var _divs$keys$j=Object(slicedToArray["a" /* default */])(divs[keys[j]],3),container=_divs$keys$j[0],children=_divs$keys$j[1],watch=_divs$keys$j[2];if(container){if(container.length)container=Object(toConsumableArray["a" /* default */])(container);else container=[container];if(watch){var textArr=[];checkContainer(container,children,function(ele){return textArr.push({text:ele.innerText,open:false});});scrapeObj[keys[j]]=textArr;// Listener for when the div changes
// We can prevent sending a bunch of data with a debouncer and this data is not urgent
var timer=null,cancelTimer=null;container.forEach(function(subContainer){return Object(utility["a" /* observeDOM */])(subContainer,function(){if(cancelTimer)cancelTimer();timer=setTimeout(function(){var textArr=[];checkContainer(container,children,function(ele){var text=ele.innerText;textArr.push({text:text,open:text.indexOf("\n")!==-1?true:false});});chromePort["a" /* default */].postMessage({type:"updatePeopleAlsoAsk",data:{timeStamp:timeStamp,peopleAlsoAsk:textArr}});},500);cancelTimer=function cancelTimer(){return clearTimeout(timer);};});});}else{var _textArr=[];checkContainer(container,children,function(ele){return _textArr.push(ele.innerText);});scrapeObj[keys[j]]=_textArr;}}else scrapeObj[keys[j]]=[];};for(var j=keys.length-1;j>=0;--j){_loop2(j);}return scrapeObj;};// Scrape the divs to send to the background script
